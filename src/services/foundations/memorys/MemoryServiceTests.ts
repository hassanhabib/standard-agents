...
export interface MemoryBrokerMock {
  readonly selectMemories: Mock;
  readonly insertMemory: Mock;
}
...
export function createMemoryServiceTests(): {
  memoryBrokerMock: MemoryBrokerMock;
  loggingBrokerMock: LoggingBrokerMock;
  memoryService: MemoryService;
} {
  const memoryBrokerMock: MemoryBrokerMock = { selectMemories: vi.fn(), insertMemory: vi.fn() };
  const loggingBrokerMock = createLoggingBrokerMock();

  const memoryService = new MemoryService(
    memoryBrokerMock as unknown as MemoryBroker,
    loggingBrokerMock as unknown as LoggingBroker,
  );

  return { memoryBrokerMock, loggingBrokerMock, memoryService };
}
