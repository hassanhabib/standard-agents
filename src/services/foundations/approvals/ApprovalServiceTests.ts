...
export interface ApprovalBrokerMock {
...
export function createApprovalServiceTests(): {
  approvalBrokerMock: ApprovalBrokerMock;
  loggingBrokerMock: LoggingBrokerMock;
  approvalService: ApprovalService;
} {
  const approvalBrokerMock: ApprovalBrokerMock = { request: vi.fn() };
  const loggingBrokerMock = createLoggingBrokerMock();

  const approvalService = new ApprovalService(
    approvalBrokerMock as unknown as ApprovalBroker,
    loggingBrokerMock as unknown as LoggingBroker,
  );

  return { approvalBrokerMock, loggingBrokerMock, approvalService };
}
