...
export interface ClassifierBrokerMock {
  readonly classify: Mock;
  readonly assess: Mock;
}
...
export function createGateServiceTests(): {
  classifierBrokerMock: ClassifierBrokerMock;
  loggingBrokerMock: LoggingBrokerMock;
  gateService: GateService;
} {
  const classifierBrokerMock: ClassifierBrokerMock = { classify: vi.fn(), assess: vi.fn() };
  const loggingBrokerMock = createLoggingBrokerMock();

  const gateService = new GateService(
    classifierBrokerMock as unknown as ClassifierBroker,
    loggingBrokerMock as unknown as LoggingBroker,
  );

  return { classifierBrokerMock, loggingBrokerMock, gateService };
}
