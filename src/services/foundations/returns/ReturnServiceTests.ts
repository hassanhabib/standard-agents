...
export function createReturnServiceTests(): {
  loggingBrokerMock: LoggingBrokerMock;
  returnService: ReturnService;
} {
  const loggingBrokerMock = createLoggingBrokerMock();
  const returnService = new ReturnService(loggingBrokerMock as unknown as LoggingBroker);

  return { loggingBrokerMock, returnService };
}
