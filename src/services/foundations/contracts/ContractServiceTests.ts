...
export interface ContractBrokerMock {
  readonly validate: Mock;
}
...
export function createContractServiceTests(): {
  contractBrokerMock: ContractBrokerMock;
  loggingBrokerMock: LoggingBrokerMock;
  contractService: ContractService;
} {
  const contractBrokerMock: ContractBrokerMock = { validate: vi.fn() };
  const loggingBrokerMock = createLoggingBrokerMock();

  const contractService = new ContractService(
    contractBrokerMock as unknown as ContractBroker,
    loggingBrokerMock as unknown as LoggingBroker,
  );

  return { contractBrokerMock, loggingBrokerMock, contractService };
}
