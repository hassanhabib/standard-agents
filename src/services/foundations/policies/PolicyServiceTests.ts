...
export interface PolicyBrokerMock {
...
export function createPolicyServiceTests(): {
  policyBrokerMock: PolicyBrokerMock;
  loggingBrokerMock: LoggingBrokerMock;
  policyService: PolicyService;
} {
  const policyBrokerMock: PolicyBrokerMock = { authorize: vi.fn(), mentions: vi.fn() };
  const loggingBrokerMock = createLoggingBrokerMock();

  const policyService = new PolicyService(
    policyBrokerMock as unknown as PolicyBroker,
    loggingBrokerMock as unknown as LoggingBroker,
  );

  return { policyBrokerMock, loggingBrokerMock, policyService };
}
...
export function createRandomEffect(): AgentEffect {
  return createAgentEffect(createRandomString(), createRandomString(), createRandomString());
}
